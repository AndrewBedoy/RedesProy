...
export class SesionComponent {
  ...
  async enviarDatosAlServidor(datos: any): Promise<void> {
    const url = 'http://192.168.50.75:3000/registro';
    const opciones = {
      method: 'POST',
      headers: {
  ...
  async registrar() {
    ...
    try {
      const url = 'http://192.168.50.75:3000/registro/idUltimoRegistro';
      const opciones = {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  async iniciarSesion(): Promise<boolean> {
    try {
      ...
      const url = 'http://192.168.50.75:3000/registros'; // Reemplaza con la URL correcta de tu servidor.
      const opciones = {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  async obtenerAdministradores(idUsuario: number, usuario: any): Promise<boolean> {
    try {
      const url = `http://192.168.50.75:3000/administradores/${idUsuario}`;
  ...
  async obtenerDoctores(idUsuario: number, usuario: any): Promise<boolean> {
    try {
      const url = `http://192.168.50.75:3000/doctores/${idUsuario}`;
      const response = await fetch(url);
      const data = await response.json();
